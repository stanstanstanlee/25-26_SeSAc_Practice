...
*/
const getRecipeByMeal = async (meal) => {
  const { data } = await axios.get(`${BASE_URL}/recipes/meal-type/${meal}`);
  console.log(data);
};
// getRecipeByMeal("lunch");
...
const getUserByName = async (name) => {
  const { data } = await axios.get(`${BASE_URL}/users/search?q=${name}`);
  console.log(data);
};
...
const getTodosByUserId = async (userId) => {
  const { data } = await axios.get(`${BASE_URL}/todos/user/${userId}`);
  console.log(data);
};
