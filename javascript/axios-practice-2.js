...
const getProductAvgRatingById = async (id) => {
  const res = await axios.get(`${BASE_URL}/products/${id}`);
  const data = res["data"];
  const reviews = data["reviews"];
...
*/
const getPostsTagList = async () => {
  const res = await axios.get(`${BASE_URL}/posts/tag-list`);
  const data = res.data;
  console.log(data);
};
// getPostsTagList();
...
const getPostByTag = async (tag) => {
  const { data } = await axios.get(`${BASE_URL}/posts/tag/${tag}`);
  console.log(data);
};
// getPostByTag("life");
...
7
*/
const getProductsTotalNum = async (productName) => {
  const { data } = await axios.get(
    `${BASE_URL}/products/search?q=${productName}`
  );
  const { total } = data;
  console.log(total);
};

getProductsTotalNum("glass");
